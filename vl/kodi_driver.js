...
function KODI()
{
   ...
   var listeners = {};
	...
	var device = IR.CreateDevice(IR.DEVICE_CUSTOM_TCP, "DeviceKodi", "127.0.0.1", 9090);
	device.Disconnect();
	...
	{ 
		//IR.Log("Data received=" + text + "|len:" + text.length);
		var start, count, response, callback, open, close;
      // Добавляем к тексту в буфере
      buffer += text;
      
		count = 0;
      open = buffer.indexOf('{');
      close = buffer.indexOf('}');
      while(open != close)
      {
