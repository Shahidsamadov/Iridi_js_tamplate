...
function Audio(gui)
{
   ...
   function Init()
   {
      ...
      IR.AddListener(IR.EVENT_ITEM_SELECT, list, function (item, subItem)
      {
         ...
         } else if(subItem == 6)
         {
            // Чекбокс - пометить
            select[item] ^= 1;
            list.CreateItem(item, 0, {FillColor: (select[item] ? gui.contentCol1 : gui.contentCol0)});      
      ...
      function new_filter(){
         ...
         var clear = GetItem(gui.audioclearSearch); 
         if(list.Filter == "") clear.Visible = clear.Enable = 0;
         else clear.Visible = clear.Enable = 1;
      }
      IR.AddListener(IR.EVENT_ITEM_CHANGE, GetItem(gui.audioFilter), new_filter); 
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioclearSearch), new_filter); 
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audiohideSearch), new_filter); 
      ...
      // Кнопка добавить
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioAdd), AddToPlaylist);

      // Кнопка заменить
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioReplace), function()
      {
      ...
      // Кнопка очистить
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioClear), function()
      {
         xbmc.Request("Playlist.Clear", {playlistid: 0});
         UpdatePlayList();
      });
      ...
      // Кнопка обновить
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioRefresh), function()
      {
         UpdatePlayList();
      });
      ...
      // Кнопка Next
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioNext), function()
      {
      ...
      // Кнопка Previous
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioPrev), function()
      {
      ...
      {
         percent.Value = volume.Value;         
         xbmc.Request("Application.SetVolume", {"volume": volume.Value});
      });
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioPlus), function()
      {
         volume.Value += 5;
         percent.Value = volume.Value;        
         xbmc.Request("Application.SetVolume", {"volume": volume.Value});
      ...
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioMinus), function()
      {
         volume.Value -= 5;
         percent.Value = volume.Value;     
         xbmc.Request("Application.SetVolume", {"volume": volume.Value});
      ...
      timepast = GetItem(gui.audioTimePast);
      ...
      IR.AddListener(IR.EVENT_ITEM_RELEASE, shuffle, function()
      {
         xbmc.Request("Player.SetShuffle", {"playerid" : 0, "shuffle": shuffle.Value ? true : false});
         UpdatePlayList();
      });
      
      repeat = GetItem(gui.audioRepeat);
      IR.AddListener(IR.EVENT_ITEM_RELEASE, repeat, function()
      {
         repeat.State = (repeat.State + 1) % 3;
         xbmc.Request("Player.SetRepeat", {"playerid": 0, "repeat": mode[repeat.State]});
      });      
      
      // Выбор типа контента
      //popup = IR.GetPopup("AudioSelect");
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioSelectAlbum), function()
      {
         history = [];
         UpdateContent("album", {properties: ["artist", "thumbnail", "year"]});
      });	
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioSelectArtist), function()
      {
         history = [];
         UpdateContent("artist", {});
      });	
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioSelectGenre), function()
      {
         history = [];
         UpdateContent("genre", {});
      });	
      IR.AddListener(IR.EVENT_ITEM_RELEASE, GetItem(gui.audioSelectFolder), function()
      {
         history = [];
   ...
   */
   function SetContentIcon(type)
   {
      ...
      back.GetState(1).Image = icon_map[type];
   }
   ...
   */
   ...
   function UpdateContent(ntype, filter)
   {
      ...
      xbmc.Request(map[type].method, filter, function(response)
      {
         try {
            ...
            {        
               ...
               for(var i = 0; i < items.length; ++i)
               {
                  ...
                  {
                  case "artist": 
                   list.Template = "XBMC_AudioListTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(5).Image}); 
                     break;
                  case "album": 
                   list.Template = "XBMC_AudioListTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(6).Image});   
                  ...
                  case "genre": 
                   list.Template = "XBMC_AudioListTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(10).Image});   
                     break;
                  case "source": 
                     list.Template = "XBMC_FolderTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(11).Image});   
                  ...
                  case "song": 
                  list.Template = "XBMC_AudioListTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(1).Image});  
                     list.CreateItem(i, 3, {Text: items[i].artist});                  
                     //date.setTime(items[i].duration * 1000);
                     list.CreateItem(i, 5, {Text: "" + CreateDataString(items[i].duration) }); //date.getUTCMinutes() + ((date.getUTCSeconds() < 10) ? ":0" : ":") + date.getUTCSeconds()});
                     break;
                  case "file": 
                     list.Template = "XBMC_FolderTemplate";
                     list.CreateItem(i, 4, {Image: GetItem(gui.iconMap).GetState(1).Image});  
         ...
         } catch(error) { IR.Log("UpdateContent::Exception catch!"); debug(error); }
   ...
   function UpdatePlayListCallback(response)
   {
      ...
      if(items !== undefined)
      {
         ...
         for(var i = 0; i < items.length; ++i)
         {
            ...
            playlist.CreateItem(id, 5, {Text: "" + CreateDataString(items[i].duration) }); /*date.getUTCMinutes() + ((date.getUTCSeconds() < 10) ? ":0" : ":") + date.getUTCSeconds()*/               
            if(items[i].thumbnail)
   ...
   function UpdatePlayer()
   {
      ...
      xbmc.Request("Player.GetItem", {"playerid" : 0, properties: ["duration", "album", "artist", "title", "thumbnail"]}, function(response)
      {
         if(response.result)
         {
            ...
            artist.Text = (response.result.item.artist) ? response.result.item.artist : "";
            album.Text = (response.result.item.album) ? response.result.item.album : "";
      ...
      xbmc.Request("Player.GetProperties", {"playerid" : 0
      , properties: ["time", "totaltime", "speed", "position", 
                     "repeat", "shuffled"]}
      , function(response)
      {
         if(response.result)
         {                        
            ...
            timepast.Text = "" + time.hours + ((time.minutes < 10) ? ":0" : ":") + time.minutes + ((time.seconds < 10) ? ":0" : ":") + time.seconds;            
            ...
            {
               playlist.CreateItem(current, 0, {FillColor: gui.playlistCol0});
               current = response.result.position;
               playlist.CreateItem(current, 0, {FillColor: gui.playlistCol1});
            }
         }
      });  
   }
   ...
   function UpdateVolume()
   {
      ...
      {
         percent.Value = response.result.volume;
         volume.Value = response.result.volume;
         mute.Value =  (response.result.muted ^ 1) ? 0 : 1;
      });  
   }
   ...
   function Notify(response)
   {
      ...
      { 
         ...
         // Плеер
         else if(response.method.indexOf("Player.") >= 0 && response.params.data.player.playerid == 0)
         {
            ...
            case "Player.OnPropertyChanged":               
               if(response.params.data.property.shuffled != undefined)
                  shuffle.Value = response.params.data.property.shuffled ? 1 : 0;
               if(response.params.data.property.repeat != undefined)
                  repeat.State = (response.params.data.property.repeat == "off") ? 0 : ((response.params.data.property.repeat == "all") ? 1 : 2);
               break;
            }
         }
         // Плейлист 
         else if(response.method.indexOf("Playlist.") >= 0 && 
         ...
         {
            percent.Value = response.params.data.volume;
   ...
   function Work(time)
   {
      ...
      if(play.Value)
      {
         if(position < duration)
         {
            ...
            timepast.Text = "" + CreateDataString(position); //date.getUTCHours() + ((date.getUTCMinutes() < 10) ? ":0" : ":") + date.getUTCMinutes() + ((date.getUTCSeconds() < 10) ? ":0" : ":") + date.getUTCSeconds();
            //date.setTime((duration - position) * 1000);                      
            timeleft.Text = "-" + CreateDataString(duration - position);//date.getUTCHours() + ((date.getUTCMinutes() < 10) ? ":0" : ":") + date.getUTCMinutes() + ((date.getUTCSeconds() < 10) ? ":0" : ":") + date.getUTCSeconds(); 
         } else
         {
            ClearInformation();
         }     
   ...
   function ClearInformation(){
         art.GetState(0).Image = track.Text = '';
   ...
   function ShowWaitPopup() {page.ShowPopup(gui.wait);}
   function HideWaitPopup() {page.HidePopup(gui.wait);}
   
   function ShowHidePopup() {page.ShowPopup(gui.hide_popup);}
   function HideHidePopup() {page.HidePopup(gui.hide_popup);}
};
