...
{            
   ...
   //NvidiaShield
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupMainTV").GetItem("nvidia"), function() {
      sourceSelect.selectNvidiaShieldTV();
   });
   ...
   //NvidiaShield
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupMainProj").GetItem("nvidia"), function() {
      sourceSelect.selectNvidiaShieldPR();
   });
   ...
   ///Nvidia
   var smarttvMoveControl = new MoveControl("PopupNvidia", "joy", "Global Cache");
   smarttvMoveControl.setCMD("CURSOR UP", "CURSOR DOWN", "CURSOR LEFT", "CURSOR RIGHT", "CURSOR ENTER");               
   smarttvMoveControl.setVisualizeItems("visual_up", "visual_down", "visual_left", "visual_right", "visual_press");
   
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupNvidia").GetItem("back"), function() {
      IR.Log("back");
      openMainPopup();
   });
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupNvidia").GetItem("set"), function() {
      openActualSettings();
   });
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupNvidia").GetItem("main"), function() {
      IR.SetVariable("Global.Popup_NvidiaShield_SelectP1", 1);
      IR.SetVariable("Global.Popup_NvidiaShield_SelectP2", 0);
   });
   IR.AddListener(IR.EVENT_ITEM_RELEASE, IR.GetPopup("PopupNvidia").GetItem("add"), function() {
      IR.SetVariable("Global.Popup_NvidiaShield_SelectP1", 0);
      IR.SetVariable("Global.Popup_NvidiaShield_SelectP2", 1);
   });
   ///FM
   ...
   function openActiveSource() {
      if (IR.GetVariable("Global.SourceSAT_TV") == 1) {
      ...
      } else if (IR.GetVariable("Global.SourceNvidiaShield_TV") == 1) {
         IR.HideAllPopups();
         IR.ShowPopup("PopupNvidia");
         ...
         IR.HideAllPopups();
         IR.ShowPopup("PopupBluray");
      } else if (IR.GetVariable("Global.SourceNvidiaShield_PR") == 1) {
         IR.HideAllPopups();
         IR.ShowPopup("PopupNvidia");
